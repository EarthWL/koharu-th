...
async fn dispatch(method: Method, params: rmpv::Value, state: AppResources) -> Result<rmpv::Value> {
    match method {
        ...
        Method::EngineProfileClearSetting => {
            call(operations::engine_profile_clear_setting, state, params).await
        }
        ...
        Method::SessionHistoryState => call(operations::session_history_state, state, params).await,
        Method::SessionHistoryRecent => {
            call(operations::session_history_recent, state, params).await
        }
