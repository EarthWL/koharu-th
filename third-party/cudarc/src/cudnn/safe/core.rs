...
impl Drop for Cudnn {
    fn drop(&mut self) {
        ...
        if !handle.is_null() {
            // KOHARU-TH PATCH (KI-1): upstream `.unwrap()`s this Result.
            // candle caches the Cudnn handle in thread-local storage, so
            // Drop runs during thread/TLS teardown. If cudnnDestroyHandle
            // returns CUDNN_STATUS_INTERNAL_ERROR there (common when the
            // CUDA primary context is already tearing down at shutdown),
            // the panic fires inside a destructor during TLS cleanup —
            // above every catch_unwind boundary — and Rust aborts the
            // whole process (`STATUS_STACK_BUFFER_OVERRUN` on Windows).
            // Swallowing the destroy error is safe: the process is going
