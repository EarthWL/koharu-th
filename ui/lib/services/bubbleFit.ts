...
const OVAL_BUBBLE_CORRECTION = 0.8
...
export function bubbleFitWarning(block: TextBlock): BubbleFitWarning | null {
  ...
  const fontSize =
    block.style?.fontSize && block.style.fontSize > 0
  ...
  const minReadableCapacity =
    usableArea / (minReadableFontSize * 0.42 * minReadableFontSize * 1.1)
  ...
  if (!isAuto) {
    ...
    const TIGHT_FILL = 0.8

    if (fill >= OVERFLOW_FILL) {
      level = 'overflow'
      reason = i18n.t('bubbleFit.overflowLocked', {
        fill: Math.round(fill * 100),
        fontSize: Math.round(fontSize),
      })
    } else if (fill >= TIGHT_FILL) {
      level = 'tight'
      reason = i18n.t('bubbleFit.tightLocked', {
        fill: Math.round(fill * 100),
        fontSize: Math.round(fontSize),
      })
    } else {
    ...
    if (wouldBeTooSmall) {
      level = 'overflow'
      const estimatedAutoFs = Math.max(
        5,
        Math.round((usableArea / (effectiveLength * 0.42 * 1.1)) ** 0.5),
      )
      reason = i18n.t('bubbleFit.overflowAutoTooSmall', {
        fontSize: estimatedAutoFs,
      })
    } else if (fill >= 1.0 || ratio >= OVERFLOW_RATIO) {
      level = 'overflow'
      reason = i18n.t('bubbleFit.overflowAutoLarge', {
        fill: Math.round(fill * 100),
      })
