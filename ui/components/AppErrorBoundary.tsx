  ...
  return (
    <div className='flex h-full min-h-screen w-full flex-col items-center justify-center bg-zinc-950 p-6 text-center select-none'>
      <div className='w-full max-w-2xl rounded-3xl border border-red-500/25 bg-zinc-900/60 p-8 shadow-[0_25px_80px_rgba(0,0,0,0.5)] backdrop-blur-xl'>
        ...
        <div className='mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-red-500/10 text-red-500 shadow-[0_0_20px_rgba(239,68,68,0.25)]'>
          <AlertTriangle className='h-8 w-8 animate-pulse text-red-500' />
        </div>

        <h1 className='mb-2 text-2xl font-bold tracking-tight text-red-500 dark:text-red-400'>
          แอปพลิเคชันขัดข้อง (Application Crashed)
        </h1>
        <p className='mx-auto mb-6 max-w-md text-sm text-zinc-400'>
          เกิดข้อผิดพลาดรุนแรงในการประมวลผลอินเทอร์เฟซ
          คุณสามารถลองกู้คืนระบบหรือส่งรายงานข้อผิดพลาดให้ทีมพัฒนา
        </p>

        {/* Diagnostic info */}
        <div className='mb-6 overflow-hidden rounded-2xl border border-white/5 bg-black/40 text-left'>
          <div className='flex items-center justify-between border-b border-white/5 bg-white/5 px-4 py-3'>
            ...
            <div className='flex gap-2'>
              <Button
                ...
                onClick={handleCopy}
                className={cn(
                  'h-7 px-3 text-[11px] font-semibold text-zinc-400 transition-colors duration-200 hover:text-white',
              ...
              <Button
                ...
                className='h-7 px-3 text-[11px] font-semibold text-zinc-400 transition-colors duration-200 hover:text-white'
              >
          ...
          <div className='scrollbar-thin scrollbar-thumb-zinc-800 max-h-48 overflow-y-auto p-4 pr-1 font-mono text-[11px] leading-relaxed break-all whitespace-pre-wrap text-red-400/90 select-text'>
            <div className='mb-1 font-bold'>{errorMessage}</div>
            <div className='mt-2 border-t border-white/5 pt-2 text-zinc-500'>
              {errorStack}
            </div>
          </div>
        </div>
        ...
        <div className='flex flex-wrap items-center justify-center gap-3 border-t border-white/5 pt-6'>
          ...
          <Button
            size='sm'
            variant='outline'
            className='border-white/10 text-xs font-semibold text-zinc-300 hover:bg-white/5 hover:text-white'
            onClick={() => {
